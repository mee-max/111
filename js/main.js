...
d3.csv('../data/weather.csv', data => {
  ...
  data.forEach(d => {
    ...
    handledData[state][year].total += prcp;
    handledData[state][year].monthly[month - 1] += prcp;
  });
  ...
  **/
  ...
  .insert("div")
  .attr("id", "main")
  .attr("width", rectWidth + margin.left + margin.right + 200)
  .attr("height", rectHeight + margin.top + margin.bottom)


  let left = d3.select("#main")
  .insert("div")
  .attr("id", "left")
  .attr("width", 200)
  .style("float", "left")
  ...
  const colorLinear = ['#ECF5FF', '#D2E9FF', '#C4E1FF', '#ACD6FF', '#97CBFF', '#84C1FF', '#66B3FF', '#46A3FF', '#2894FF', '#0080FF', '#0072E3', '#0066CC', '#005AB5', '#004B97']
    ...
    for(let state in handledData) {
      json.features.some(jsonFeature => {
        if (state === jsonFeature.properties.name) {
          jsonFeature.properties.value = handledData[state][currentYear].total/count[state].value;
          return true;
        }
        return false;
      })
